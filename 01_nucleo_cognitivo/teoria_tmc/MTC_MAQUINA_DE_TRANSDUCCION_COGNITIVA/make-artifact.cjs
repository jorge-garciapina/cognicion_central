...
function shouldIgnore(relPath, dirent, gitignorePatterns) {
  // El archivo de salida nunca debe serializarse dentro de sí mismo.
  const absoluteCandidate = path.resolve(ROOT, relPath);
  if (absoluteCandidate === OUT_PATH) return true;

  const base = path.basename(relPath);
  if (DEFAULT_IGNORES.has(base)) return true;

  // Los PDF son portadores visuales pesados y quedan fuera del artefacto textual.
  if (!dirent.isDirectory() && IGNORED_EXTENSIONS.has(path.extname(base).toLowerCase())) {
    return true;
  }
